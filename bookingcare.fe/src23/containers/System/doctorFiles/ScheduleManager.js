...
import vi from 'date-fns/locale/vi'; //v84xx1
import { scheduleManagerLangs } from '../../../connectSupplyFE/otherSupplies';
import 'dotenv/config'; //19ms08ss
...
class ScheduleManager extends Component {
  ...
  renderPickerDate = () => {
    ...
    return (
      <DatePicker
        ...
        minDate={new Date()}
        dateFormat={customDatePicker.formatLong.date()} //v84xx3
        locale={
          this.props.language === LANGUAGES.VI
            ? { ...vi, ...customDatePicker } //v84xx3
            : ''
        }

        /*v84xx1*/
        // dateFormat='dd/MM/yyyy'
        // locale={this.props.language === LANGUAGES.VI ? vi : ''}
      />
    );
  };
  ...
  handleSchedule = () => {
    ...
    const maxNumber = process.env.MAX_NUMBER_SCHEDULE || 10; //19ms08ss
  ...
  submitSchedule = async () => {
    ...
    if (schedulesArr && schedulesArr.length > 0) {
      ...
      const data = await uploadDoctorScheduleFn(schedulesArr); //v83xx1
      if (data.errCode === 0) {
        this.clearForm();
      }
    }
...
const mapDispatchToProps = (dispatch) => {
  return {
    uploadDoctorScheduleFn: (dataArr) =>
      dispatch(actions.uploadDoctorScheduleFn(dataArr)), //v83xx1
